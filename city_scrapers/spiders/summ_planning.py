...
class SummPlanningSpider(CityScrapersSpider):
    ...
    start_urls = [
        "https://co.summitoh.net/index.php/departments/community-a-economic-development/planning"  # noqa
    ]
        ...
        for link in section.css("a"):
            ...
            if "Dates" in link_text:
                yield response.follow(
                    link.attrib["href"],
                    callback=self._parse_pdf_schedule,
                    dont_filter=True,
                )
                break
    ...
    def _parse_section(self, response):
        ...
        accordion_el = " ".join(
            response.css(".modulebrd > div > div > div > *").extract()
        )
    ...
    def _parse_links_page(self, section, response):
        ...
        for item in section.css("body > *"):
            ...
            links_date_map[start_date].append({"title": link_title, "href": link_href})
